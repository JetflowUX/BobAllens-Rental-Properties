...
export function Layout() {
  ...
  return (
    <div className="min-h-screen flex flex-col">
      <header
        className={`fixed top-0 w-full z-50 transition-all duration-500 ${isScrolled ? 'bg-cream/90 backdrop-blur-md border-b border-divider py-4' : 'bg-transparent py-6'}`}>
        ...
        <div className="max-w-7xl mx-auto px-6 md:px-12 flex justify-between items-center">
          <Link to="/" className="group flex flex-col">
            ...
            <span className="font-sans text-[0.65rem] uppercase tracking-[0.2em] text-warmgray group-hover:text-olive transition-colors">
              Rental Properties
            </span>
          </Link>
          ...
          <nav className="hidden md:flex items-center space-x-10">
            ...
            <Link
              key={link.name}
              to={link.path}
              className="font-sans text-sm uppercase tracking-widest text-charcoal hover:text-olive transition-colors">
      ...
      {/* Mobile Nav */}
      <div
        className={`fixed inset-0 bg-cream z-40 transition-transform duration-500 ease-in-out md:hidden flex flex-col justify-center items-center ${mobileMenuOpen ? 'translate-y-0' : '-translate-y-full'}`}>
        
        <nav className="flex flex-col items-center space-y-8">
          <Link to="/" className="font-serif text-4xl text-charcoal mb-8">
            Home
          </Link>
          {navLinks.map((link) =>
          <Link
      ...
      <footer className="bg-charcoal text-cream pt-24 pb-12 px-6 md:px-12">
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-4 gap-12 md:gap-8 mb-24">
          <div className="md:col-span-2">
            <h3 className="font-serif text-3xl mb-4">Bob Allen's</h3>
        ...
        <div className="max-w-7xl mx-auto border-t border-warmgray/30 pt-8 flex flex-col md:flex-row justify-between items-center gap-4">
